...
(function (root) {
  ...
  function baseYs(p) {
    ...
    var a = Math.max(12, p.hillWidth * 0.10);
    var travel = Math.max(0, p.hillWidth - 2 * a - p.baseW);
    var out = [], i;
    for (i = 0; i < n; i++) {
  ...
  function ribCuts(p) {
    ...
    if (p.hillWidth <= bed) return [[0, p.hillWidth]];
    var ys = baseYs(p), mids = ys.map(function (r) { return (r[0] + r[1]) / 2; });
    var cuts = [0], i, guard = 0;
    while (p.hillWidth - cuts[cuts.length - 1] > bed && guard++ < 50) {
